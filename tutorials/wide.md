...
请注意，哈希碰撞是可能的，但可能不会显着影响模型质量。在底层实现上，`LinearModel` 类负责管理映射并创建 `tf.Variable` 来存储每个特征 ID 的模型参数（也称为模型权重）。模型参数将通过后面将要讨论的模型训练过程来学习。
...
1.  随着年龄增长，收入总是以某种速度减少（负相关），或者
1.  无论年龄多少，收入都保持不变（不相关）

如果我们想分别学习收入和各个年龄组之间的细微关联，我们可以利用 **bucketization**。 分桶是将连续特征的整个范围划分为一组连续的桶，然后根据该值落入哪个桶将原始数值特征转换为桶 ID（作为类别型特征）。
