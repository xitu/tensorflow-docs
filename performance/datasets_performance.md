...
一个典型的 TensorFlow 输入管道训练过程可以被设计为一个 ETL 流程：
...
`tf.data` API 为用户提供构建块以构建有效利用 CPU 的输入管道，优化 ETL 流程的每个步骤。
...
为此， `tf.data`  API 提供了@ {tf.contrib.data.map_and_batch} 转换，它有效地"融合“了地图和批量转换。
...
数据的每次重复通常被称为迭代次数。@ {tf.data.Dataset.shuffle} 转换将乱序随机化数据集样例的顺序。
