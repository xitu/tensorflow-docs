...
当准备批处理时，可能需要预处理输入元素。为此，`tf.data ` API 提供了 @{tf.data.Dataset.map} 转换，该转换将用户定义的函数（例如，来自运行示例的 `parse_fn` ）应用于输入数据集的每个元素。由于输入元素彼此独立，因此可以在多个CPU内核之间对预处理进行并行处理。为了实现这一点， `map` 转换提供了`num_parallel_calls` 参数来指定并行性的级别。例如，下图说明了将 `num_parallel_calls=2`设置到 `map` 转换的效果：
